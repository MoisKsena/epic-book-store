function ready(fn) {
  if (document.attachEvent ? document.readyState === "complete" : document.readyState !== "loading"){
    fn();
  } else {
    document.addEventListener('DOMContentLoaded', fn);
  }
}

ready(function(){
  
	const data = {
      item:[
        {
       	  image: 'img/book-lateralnaya-logika.jpg',
          name: 'Латеральная логика',
          descr: 'Головоломный путь к нестандартному мышлению',
          price: '500'
        },
        {
       	  image: 'img/book-putevoditel-po-lzhi.jpg',
          name: 'Путеводитель по лжи',
          descr: 'Критическое мышление в эпоху постправды',
          price: '630'
        },
        {
       	  image: 'img/book-dao-fiziki.jpg',
          name: 'Дао физики',
          descr: 'Исследование параллелей между современной физикой и восточной философией',
          price: '560'
        },
        {
       	  image: 'img/book-ne-ochevidno.jpg',
          name: 'Не очевидно',
          descr: 'Как выявлять тренды раньше других',
          price: '365'
        },
        {
       	  image: 'img/book-superpotrebiteli.jpg',
          name: 'Суперпотребители',
          descr: 'Кто это и почему они так важны для вашего бизнеса',
          price: '500'
        },
        {
       	  image: 'img/book-social-media-marketing.jpg',
          name: 'Маркетинг в социальных сетях',
          descr: '',
          price: '630'
        },
        {
       	  image: '/img/book-pravilnyij-vyibor.jpg',
          name: 'Правильный выбор',
          descr: 'Практическое руководство по принятию взвешенных решений',
          price: '560'
        },
        {
       	  image: 'img/book-glavnoe-v-istorii-iskusstv.jpg',
          name: 'Главное в истории искусств',
          descr: 'Ключевые работы, темы, направления, техники',
          price: '365'
        }
      ]
    };

      const cardsWrap = document.querySelector('.js-catalog__list');

      function createCard(item) {
        const card = document.createElement('div');
        card.classList.add('product-card-mini');

        card.innerHTML = `<a href="" class="product-card-mini__img-wrap"><img src="${item.image}" alt="" class="product-card-mini__img"></a>
					<h2 class="product-card-mini__title">${item.name}</h2>
					<p class="product-card-mini__descr">${item.descr}</p>
					<div class="product-card-mini__price">${item.price} &#8381;</div>`;

        return card;
      };

      function insertElements(obj, wrap) {
        const membersArr = obj.item;

        membersArr.forEach((item) => {
          const card = createCard(item);

          wrap.appendChild(card);
        })
      };

      if (cardsWrap) {
        insertElements(data, cardsWrap);
      };

});
